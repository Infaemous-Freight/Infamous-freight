...
describe('Netlify production routing', () => {
  ...
  it('keeps the browser-critical API proxy ahead of the SPA fallback in netlify.toml', () => {
    ...
    const apiHostSocketProxy = indexOf(rootContent, 'from = "https://api.infamousfreight.com/socket.io/*"');
    ...
    expect(apiHostSocketProxy).toBeLessThan(socketProxy);
    ...
    expect(rootContent).toContain('from = "https://api.infamousfreight.com/socket.io/*"\n  to = "https://infamous-freight-api.fly.dev/socket.io/:splat"\n  status = 200\n  force = true');
