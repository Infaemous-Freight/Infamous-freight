...
async function enforceTransitionPrerequisites(tx, job, to, actor, data) {
    switch (to) {
        ...
        case "DELIVERED":
            require(
                job.status === "PICKED_UP" || job.status === "ACCEPTED",
                JOB_TRANSITION_CODES.ILLEGAL_TRANSITION,
                "Only picked-up/accepted jobs can be delivered"
            );
...
async function transitionJob(input) {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (!job) throw new JobTransitionError(JOB_TRANSITION_CODES.JOB_NOT_FOUND, "Job not found", 404);
        ...
        if (to === "HELD") {
            ...
            if (updated.count === 0) {
                throw new JobTransitionError(JOB_TRANSITION_CODES.ALREADY_ASSIGNED, "Job already held or taken", 409);
            }
            ...
            const updated = await tx.job.updateMany({
                where: {
                    ...
                    driverId: null,
                    OR: [
                        { status: "OPEN", OR: [{ heldUntil: null }, { heldUntil: { lt: now } }] },
                        { status: "HELD", heldByDriverId: actor.userId, heldUntil: { gt: now } },
                    ],
                },
                data: buildStatusUpdatePayload(job, to, actor, data),
            });
            if (updated.count === 0) {
                throw new JobTransitionError(JOB_TRANSITION_CODES.ALREADY_ASSIGNED, "Job was just accepted by another driver", 409);
            }
            updatedJob = await tx.job.findUnique({ where: { id: jobId } });
        } else {
            updatedJob = await tx.job.update({ where: { id: jobId }, data: buildStatusUpdatePayload(job, to, actor, data) });
        }
        ...
        await tx.jobEvent.create({
            data: {
                ...
                metadata: JSON.stringify({ from, to, timestamp: new Date().toISOString(), actor, data }),
            },
        });
