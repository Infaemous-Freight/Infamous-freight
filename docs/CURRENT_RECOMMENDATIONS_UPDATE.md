...
The repository now includes exact forced public API proxy routes, an exact forced `/api/health` proxy, and forced `/api/*` and `/socket.io/*` proxy rules ahead of the SPA fallback. The Netlify build configuration points functions at `netlify/event-functions` so only the intentionally enabled public freight wrapper is packaged for normal deploys; broad repo-owned HTTP functions remain outside the active Netlify functions directory. The production readiness deploy command now uses the same `netlify/event-functions` directory so CLI-triggered production deploys do not accidentally omit the public quote and tracking function. Continue using the checks from [`netlify-deploy-checklist.md`](./netlify-deploy-checklist.md) after future production deploys and append the results to the launch evidence log. Do not record secrets or environment variable values in the evidence.
